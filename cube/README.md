...
- **No pre-aggregations (v1)** — the demo dataset (~205k events, marts ≤ 42k rows) answers directly on Neon's smallest compute. Add rollups in `cube.js` when query volume demands them — until then Cube Store has nothing to do.