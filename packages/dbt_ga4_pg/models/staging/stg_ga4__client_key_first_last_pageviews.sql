...
with page_views_first_last as (
    select
        client_key,
        (array_agg(event_key order by event_timestamp asc))[1] as first_page_view_event_key,
        (array_agg(event_key order by event_timestamp desc))[1] as last_page_view_event_key
...
page_views_joined as (
    ...
    left join {{ref('stg_ga4__event_page_view')}} first_page_view
        on page_views_first_last.first_page_view_event_key = first_page_view.event_key
    left join {{ref('stg_ga4__event_page_view')}} last_page_view
        on page_views_first_last.last_page_view_event_key = last_page_view.event_key
)
