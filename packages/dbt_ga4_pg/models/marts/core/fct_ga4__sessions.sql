-- Stay mindful of performance/cost when using this model. Making this model
-- partitioned on date is not possible because there's no way to create a single
-- record per session AND partition on date. (Postgres port: full-history table,
-- rebuilt on each run — fine at demo scale.)
